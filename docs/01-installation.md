...
| `اجازهٔ نوشتن در storage/ را ندارد` | UID/GID در `.env` با مالک فایل‌ها یکی نیست — پایین را ببینید |
| `vendor/ پیدا نشد` + خطای composer | اینترنت سرور یا DNS داکر مشکل دارد |

**مشکل دسترسی (شایع‌ترین)** — وقتی پروژه را با یک کاربر کلون کرده‌اید ولی
`UID`/`GID` در `.env` چیز دیگری است:
...
> اگر با کاربر `root` کار می‌کنید، مقدار درست `UID=0` و `GID=0` است.
> این دقیقاً همان کاری است که دستور بالا انجام می‌دهد.
