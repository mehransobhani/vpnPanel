...
## راه ساده: اسکریپت خودکار

```bash
bash install.sh
```

پورت‌های آزاد را پیدا می‌کند، `.env` را می‌نویسد، سرویس‌ها را بالا می‌آورد،
نود VPN را می‌سازد و در پایان بررسی سلامت می‌کند. اگر روی همان سرور
ریورس‌پروکسی دیگری دارید، خودش پنل را روی لوکال‌هاست محدود می‌کند.
...
| `bash install.sh --new-keys` | کلیدهای REALITY را هم بازتولید می‌کند ⚠️ |

> `--new-keys` همهٔ کانفیگ‌های فروخته‌شده را بی‌اعتبار می‌کند. فقط وقتی
> استفاده کنید که کلید لو رفته باشد یا نصب را از روی پروژهٔ کس دیگری آورده‌اید.

بقیهٔ این سند مراحل را دستی توضیح می‌دهد.
...
تنظیم دیگری لازم نیست؛ کانتینر خودش دسترسی‌های `storage/` را درست می‌کند —
چه پروژه مال `root` باشد چه مال کاربر عادی.
...
**`dependency failed to start: container vpn-app-1 is unhealthy`**

علت را در لاگ ببینید — entrypoint دقیقاً می‌گوید کجا گیر کرده:
...
| پیام | راه‌حل |
|---|---|
| `فایل .env وجود ندارد` | `cp .env.example .env` |
| `اجازهٔ نوشتن در storage/ را ندارد` | `user:` را از سرویس `app` در `compose.yaml` بردارید |
| `vendor/ پیدا نشد` + خطای composer | اینترنت سرور یا DNS داکر مشکل دارد |

**مشکل دسترسی** — فقط وقتی پیش می‌آید که خودتان `user:` را به سرویس `app`
در `compose.yaml` اضافه کرده باشید. آن را بردارید تا entrypoint دسترسی‌ها
را خودش تنظیم کند.

**دیتابیس در دسترس نیست**
اگر لاگ گفت `دیتابیس ... در دسترس نیست`، مقادیر `DB_*` در `.env` را با
سرویس `mysql` در `compose.yaml` مقایسه کنید. پنل بالا می‌آید ولی کار نمی‌کند.
