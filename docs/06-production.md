...
اگر نود VPN محلی دارید، پورت آن را هم باز کنید:
...
- [ ] `.env` در بکاپ جداگانه — بدون `APP_KEY` رمزهای SSH بازیابی‌شدنی نیستند
- [ ] برای SSH نودها از **کلید** استفاده شده، نه رمز
...
پنل از ابتدا چندنودی است:

1. هر نود را طبق [سند ۰۳](03-xray-node.md) آماده کنید
2. همه را در `مدیریت ← سرورها` ثبت کنید
3. **ظرفیت** هر کدام را بگذارید تا بار پخش شود
4. در پلن‌ها تعیین کنید کدام پلن کدام کشورها را می‌گیرد

مشتری همهٔ سرورهای پلنش را در یک لینک اشتراک می‌بیند و در کلاینت بین آن‌ها سوییچ می‌کند.
